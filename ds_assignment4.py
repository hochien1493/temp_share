...
# Study the Properties of "Small World"

# 1. Generate a cycle of 1000 nodes, and each edge has length 1

#    (1) define a class of graph
...
class graph:
    ...
    def AddVertex(self):
        self.maxVertexID += 1
        self.Vertices[self.maxVertexID] = dict()
        
    
    def AddEdge(self,weight,fromID,toID):
        self.Vertices[fromID].update({toID:weight})
        self.Vertices[toID].update({fromID:weight})
        temp = [fromID,toID]
        self.Edges.append(temp)
        
    def SPT_BinaryHeap(self,src):
        self.distances = {vertex: float('infinity') for vertex in self.Vertices}
    ...
    def SPT_FibonacciHeap(self,src):
        self.distances = {vertex: float('infinity') for vertex in self.Vertices}
        self.distances[src] = 0
        pq = FibonacciHeap()
        pq.insert((0,src))
        while pq.total_nodes > 0:
            current_distance, current_vertex = pq.extract_min().key
    ...
    def CalculateAvgDist(self,src):
        self.SPT_BinaryHeap(src)
        self.AvgDist = 0
        for vertex in self.distances:
            self.AvgDist += self.distances[vertex]
        return self.AvgDist/(self.maxVertexID+1)
    
    def Visualize(self):
        G = nx.Graph()
        G.add_edges_from(self.Edges)
        nx.draw_networkx(G,with_labels = False,node_size=1,width=0.1)
        plt.show()
...
my_graph = graph(1000)
for i in range(1000):
    my_graph.AddEdge(y_prime,i%1000,(i+1)%1000)

for i in range(x):
    temp = random.sample(range(1000),2)
    my_graph.AddEdge(y,temp[0],temp[1])


sum = 0
for i in range(1000):
    temp_graph = my_graph
    sum += (temp_graph.CalculateAvgDist(i)/1000)
print(f"the average dist. of all vertices is {sum}")
# %%
